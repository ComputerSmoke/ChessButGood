...
public class Initializer : MonoBehaviour
{
    ...
    private void MakeBoard(Board board) {
        for(int i = 0; i < 8; i++) {
            for(int j = 0; j < 8; j++) {
                ...
                GameObject square = Instantiate(prefab, Pos(i, j), Quaternion.identity);
    ...
    private void PlacePiece(GameObject prefab, int col, int row, Board board) {
        GameObject piece = Instantiate(prefab, Pos(col, row), Quaternion.identity);
        board.squares[(col, row, 0)].Place(piece);
    }
    private Vector3 Pos(int col, int row) {
